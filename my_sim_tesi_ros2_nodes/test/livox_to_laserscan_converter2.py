...
class LivoxToLaserScanConverter(Node):
    ...
    def check_status(self):
        """定期检查转换器状态"""
        self.get_logger().info(f"Status: Received {self.msg_count} PointCloud2 messages so far")
        
        # 检查订阅者连接状态
        subscription_count = self.pointcloud_subscriber.get_subscription_count()
        publisher_count = self.laserscan_publisher.get_subscription_count()
        
        self.get_logger().info(f"Input connections: {subscription_count}, Output subscribers: {publisher_count}")
    ...
    def pointcloud_callback(self, msg):
        ...
        try:
            ...
            self.get_logger().info(f"=== Received PointCloud2 message #{self.msg_count} ===")
            ...
            if laser_scan is not None:
                # 统计有效点数
                valid_ranges = sum(1 for r in laser_scan.ranges if not math.isinf(r) and not math.isnan(r))
                self.get_logger().info(f"Generated LaserScan: {valid_ranges}/{len(laser_scan.ranges)} valid ranges")
                
                # 发布激光扫描数据
                self.laserscan_publisher.publish(laser_scan)
                self.get_logger().info("LaserScan published successfully!")
            else:
                self.get_logger().error("Failed to convert PointCloud2 to LaserScan")
