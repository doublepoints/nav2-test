...
echo ""
echo "2️⃣ livox_frame -> livox_frame_corrected (应该是 180° 旋转):"
timeout 3s ros2 run tf2_ros tf2_echo livox_frame livox_frame_corrected 2>/dev/null | grep -E "RPY.*degree" || echo "等待数据..."

echo ""
echo "3️⃣ base_link -> livox_frame_corrected (应该是 180° 旋转):"
timeout 3s ros2 run tf2_ros tf2_echo base_link livox_frame_corrected 2>/dev/null | grep -E "RPY.*degree" || echo "等待数据..."

echo ""
echo "💡 解释："
echo "- base_link 和 livox_frame 现在有相同的方向"
echo "- livox_frame_corrected 通过 TF 旋转 180° 来抵消数据层面的修正"  
echo "- 最终 base_link 和 livox_frame_corrected 在视觉上应该有不同的方向"
echo "- 但数据处理是正确的，因为数据和坐标系都被同时修正了"
